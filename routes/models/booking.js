...
exports.sql_list = function (req, res) {
    ...
    var sql1 = "select * from booking where type = '预定' and sendtime like '"+sendtime+"%' and addressname like '%"+_address+"%' order by bookingtime desc limit "+(page-1)*limit+","+limit;
    var sql5 = "select count(*) as count from booking where type = '预定' and sendtime like '"+sendtime+"%' and addressname like '%"+_address+"%'";
    var sql2 = "select name,b_starttime,b_endtime from address";
    var sql3 = "select numtotal from booking where type = '预定' and sendtime like '"+sendtime+"%' and addressname like '%"+_address+"%'";
        ...
        if(err){console.log(err);return false;}
          mysql.query(sql5,function (err1, rows5) {
            ...
            mysql.query(sql2,function (err2, rows2) {
                ...
                res.render('cms/booking_left', {totalnum:totalnum,s_info:s_info,address_s:rows2,url:req.url,record:rows1,page:page,total:total,totalpage:totalpage,isFirstPage:isFirstPage,isLastPage:isLastPage,info:_info});
            });
            });
          });
    });
