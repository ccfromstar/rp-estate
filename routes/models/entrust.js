...
function sql_insert(req, res) {
    ...
    var insertSql = "insert into brokers (name,job,tel,plate,keycell,selfrating,store,img) values ('"+name+"','"+job+"','"+tel+"','"+plate+"','"+keycell+"','"+selfrating+"','"+store+"','"+img+"')";
...
exports.sql_list = function (req, res) {
    ...
    var sql1 = "select * from entrust_user order by createtime desc limit "+(page-1)*limit+","+limit ;
    var sql5 = "select count(*) as count from entrust_user";
    mysql.query(sql1,function (err, rows1) {
        for(var i in rows1){
            rows1[i].createtime = (rows1[i].createtime).Format("yyyy-MM-dd hh:mm:ss");
        }
        ...
        mysql.query(sql5,function (err1, rows5) {
            ...
            var isLastPage = ((page -1) * limit + rows1.length) == total;
                res.render('cms/entrust', {url:req.url,record:rows1,page:page,total:total,totalpage:totalpage,isFirstPage:isFirstPage,isLastPage:isLastPage,info:_info});
        });
    });
};
